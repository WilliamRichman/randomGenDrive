function generate() {
    ...
    for (let i = 0; i < names.length; i++) {
        /* let name = names[i] */
        const container = Math.floor(Math.random() * (names.length - 1));
        name.push(names[container]);
        console.log(container)
        document.getElementById('target').innerHTML = container;

        /*  names.splice(container, 1)
         console.log(container) */
